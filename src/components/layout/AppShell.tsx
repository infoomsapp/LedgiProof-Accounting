...
export default function AppShell() {
  ...
  const { canViewPayroll, isAccountantFirm, workspaceRole } = useUserRole()
  ...
  // Inside a client workspace, trim to CLIENT_WORKSPACE_NAV regardless of
  // firm type — the client's own tab bar (ClientScopeBanner) already covers
  // everything else for THIS client.
  const NAV: NavItem[] = inClientWorkspace
    ? CLIENT_WORKSPACE_NAV
    : (canViewPayroll ? [...baseNav, PAYROLL_NAV] : baseNav)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--lp-bg)' }}>
          ...
          <div className="titlebar-drag" style={{
            ...
            <div style={{ fontSize: 10.5, color: 'var(--lp-text-muted)', marginTop: 5, marginLeft: 40 }}>
              ...
              {isProfessional && !isSuperAdmin && isAccountantFirm && (
                <span style={{ color: '#10b981' }}>Accountant</span>
              )}
              {isProfessional && !isSuperAdmin && !isAccountantFirm && (
                <span style={{ color: '#a78bfa' }}>Bookkeeper</span>
              )}
            </div>
          </div>
      ...
      {/* Global chat bubble — visible on all pages for professionals with an org */}
      {!isSuperAdmin && orgId && isProfessional && (
