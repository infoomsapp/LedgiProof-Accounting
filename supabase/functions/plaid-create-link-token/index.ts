...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (quotaRes.error) {
      return j({ error: `Quota check failed: ${safeMessage(quotaRes.error, 'internal error')}` }, 500)
    }
    ...
    return j({ error: safeMessage(err, 'Failed to create the bank connection link') }, 500)
  }
})
